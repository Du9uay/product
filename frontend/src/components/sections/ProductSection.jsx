...
const ProductSection = ({ type, onImageClick }) => {
  ...
  useEffect(() => {
    ...
    // 移动端：禁用IntersectionObserver，直接显示内容
    if (isMobile) {
      if (titleRef.current) {
        titleRef.current.classList.add('animate-in');
      }
      if (leftRef.current) {
        leftRef.current.style.animationPlayState = 'running';
        leftRef.current.style.opacity = '1';
        leftRef.current.style.transform = 'none';
      }
      if (rightRef.current) {
        rightRef.current.style.animationPlayState = 'running';
        rightRef.current.style.opacity = '1';
        rightRef.current.style.transform = 'none';
      }
      console.log('移动端检测：禁用ProductSection的IntersectionObserver动画');
      return;
    }
    ...
    return (
      ...
      <div className="product-content">
        <div className="product-left slide-in-left" ref={leftRef}>
          ...
          <p className="product-subdesc" style={{marginTop: '8px'}}>
